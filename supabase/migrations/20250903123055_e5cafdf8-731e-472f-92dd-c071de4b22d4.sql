...
-- Drop dependent policies first, then recreate tables properly

-- 1. Drop all existing policies that depend on wallet columns
...
-- Drop existing policies on creator_follows
...
-- 2. Recreate creator_follows with proper user-ID structure
...
-- 3. Fix nft_likes table - remove wallet dependency
...
-- New RLS policies for nft_likes
...
-- 4. Fix collection_likes table - remove wallet dependency
...
-- New RLS policies for collection_likes
...
-- 5. Add creator_user_id to nfts and collections for like counting
