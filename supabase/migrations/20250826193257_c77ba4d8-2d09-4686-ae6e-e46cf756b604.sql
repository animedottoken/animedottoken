...
-- Recreate views with proper security context (as authenticator role)
-- These will now respect RLS policies on underlying tables

-- Collection like counts view
CREATE VIEW public.collection_like_counts 
SECURITY INVOKER
AS SELECT 
    collection_id,
    count(*) AS like_count
...
CREATE VIEW public.creator_follow_stats 
SECURITY INVOKER
AS SELECT 
    creator_wallet,
...
CREATE VIEW public.creator_collection_like_stats 
SECURITY INVOKER
AS SELECT 
...
CREATE VIEW public.creator_nft_like_stats 
SECURITY INVOKER
AS SELECT 
...
CREATE VIEW public.creator_nft_likes_stats 
SECURITY INVOKER
AS SELECT 
    n.creator_address AS creator_wallet,
...
CREATE VIEW public.creators_public_stats 
SECURITY INVOKER
AS SELECT 
...
CREATE VIEW public.boosted_leaderboard 
SECURITY INVOKER
AS SELECT 
    bl.id,
    bl.nft_id,
