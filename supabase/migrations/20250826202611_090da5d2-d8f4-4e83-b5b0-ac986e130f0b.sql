...
-- (Keep existing "Users can view their own complete profile" policy)
...
-- (Keep existing "Users can view their own NFTs" policy)
...
-- (Keep existing "Users can view their own collections" policy)
...
-- Update the existing public functions to be SECURITY DEFINER to work without table access
...
COMMENT OF FUNCTION public.get_nfts_authenticated() IS 'Authenticated function providing complete NFT data with full wallet addresses for trading';
