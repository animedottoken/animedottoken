-- SECURITY HARDENING: Restrict public profile data exposure (Final)
-- Clean up and properly implement security policies

-- Step 1: Drop all existing policies on user_profiles related to public access
...
-- Step 2: Create the authenticated-only policy
...
-- Step 3: Drop and recreate all public functions with proper signatures
DROP FUNCTION IF EXISTS public.get_public_profile_safe(TEXT);
DROP FUNCTION IF EXISTS public.get_public_profile_limited(TEXT);
...
DROP FUNCTION IF EXISTS public.get_profile_authenticated(UUID);

-- Step 4: Create limited public profile function
...
-- Step 5: Create get_creators_public_explore WITHOUT wallet addresses
...
-- Step 6: Create get_collections_public_explore WITHOUT wallet addresses
...
-- Step 7: Create get_nfts_public_explore WITHOUT wallet addresses
...
-- Step 8: Create authenticated profile function
...
CREATE VIEW public.public_profiles_limited AS
SELECT 
  user_id,
  nickname,
  display_name,
