-- Create user_wallets table for Multi-Wallet Profile system (fixed version)
...
-- Create function to validate secondary wallet limit
...
-- Create trigger to enforce secondary wallet limit
CREATE TRIGGER validate_secondary_wallet_limit_trigger
BEFORE INSERT OR UPDATE ON public.user_wallets
FOR EACH ROW
EXECUTE FUNCTION public.validate_secondary_wallet_limit();
