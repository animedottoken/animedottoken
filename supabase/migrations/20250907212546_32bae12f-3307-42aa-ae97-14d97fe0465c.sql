-- Enhanced security for newsletter_subscribers table (Fixed)
...
-- 7. Create rate limiting table for newsletter operations
...
BEGIN
  ...
  window_start := date_trunc('hour', now()) - (extract(minute from now())::integer / p_window_minutes)::integer * interval '1 minute' * p_window_minutes;
