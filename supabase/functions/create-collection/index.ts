...
serve(async (req) => {
  ...
  try {
    ...
    const insertData: any = {
      ...
      // Set values based on enable_primary_sales
      mint_price: body.enable_primary_sales ? (body.mint_price ?? 0) : null,
      max_supply: body.enable_primary_sales ? (body.max_supply ?? 1000) : null,
      items_available: body.enable_primary_sales ? (body.max_supply ?? 1000) : null,
      items_redeemed: body.enable_primary_sales ? 0 : null,
      royalty_percentage: body.enable_primary_sales ? (body.royalty_percentage ?? 0) : null,
      whitelist_enabled: body.enable_primary_sales ? (body.whitelist_enabled ?? false) : null,
